...
| `/help` | **帮助菜单**<br>显示全部管理员指令说明，并提供可点击快捷按钮。 | 快速查看并一键执行指令。 |
...
7.  **⚠️ 关键步骤：绑定数据库与变量**
    ...
    * **添加环境变量**：
        ...
        * `VERIFY_MODE`: 验证模式，填 `turnstile`（默认）或 `local`（兼容旧题库）。
        * `TURNSTILE_SITE_KEY`: Cloudflare Turnstile Site Key。
        * `TURNSTILE_SECRET_KEY`: Cloudflare Turnstile Secret Key（建议使用 Secret）。
        * `TURNSTILE_VERIFY_URL`: 你的 Worker 对外地址（例如 `https://xxx.workers.dev`）。
        * `TURNSTILE_SIGN_SECRET`: 自定义签名密钥（随机长字符串，用于防伪造验证链接）。
...
6.  **配置 KV 与变量**：
    ...
    * 若使用 Turnstile，再添加：`VERIFY_MODE=turnstile`、`TURNSTILE_SITE_KEY`、`TURNSTILE_SECRET_KEY`、`TURNSTILE_VERIFY_URL`、`TURNSTILE_SIGN_SECRET`。
    * 点击 **Save and Deploy**。
